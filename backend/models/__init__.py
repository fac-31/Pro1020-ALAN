...
class UserSubscription(BaseModel):
    ...
    subscribed_at: datetime = Field(
        default_factory=datetime.now, description="Subscription date"
    )
...
class SearchRequest(BaseModel):
    ...
    user_interests: Optional[List[str]] = Field(
        None, description="User interests for filtering"
    )
    n_results: int = Field(
...
class ContentEvaluationResult(BaseModel):
    ...
    should_add: bool = Field(
        description="Whether content should be added to knowledge base"
    )
...
class DailyDigestStats(BaseModel):
    ...
    last_digest_sent: Optional[datetime] = Field(
        None, description="Last digest sent time"
    )
    next_digest_scheduled: Optional[datetime] = Field(
        None, description="Next digest scheduled time"
    )
