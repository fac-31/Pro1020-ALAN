...
class SemanticChunker(BaseChunker):
    ...
    def _select_model(self, model_name: str, model_size: str) -> str:
        ...
        if model_size == "small":
            ...
            return (
                model_name.replace("L12", "L6") if "L12" in model_name else model_name
            )
    ...
    def chunk(
        self, text: str, metadata: Optional[Dict] = None, pretokenized: bool = False
    ) -> List[Dict]:
        if metadata is None:
            metadata = {}
        ...
        try:
            ...
            if self.threshold_type == "percentile" and len(sentences) > 1:
                ...
                current_similarity_threshold = np.percentile(
                    consecutive_similarities, self.threshold_percentile
                )
            ...
            for i, (sent, emb) in enumerate(zip(sentences, sentence_embeddings)):
                ...
                last_emb_array = (
                    current_chunk_embs[-1].reshape(1, -1).astype(np.float32)
                )
                ...
                if (
                    sim < current_similarity_threshold
                    or current_length >= self.max_chunk_tokens
                ):
                    chunks.append(
                        {"text": current_chunk_text, "metadata": metadata.copy()}
                    )
                    ...
                    current_chunk_embs = (
                        current_chunk_embs[-self.overlap :] if self.overlap > 0 else []
                    )
