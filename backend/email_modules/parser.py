...
class EmailParser:
    ...
    def parse_email_message(self, raw_email: bytes) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(
                "Parsing email message, raw_email type: %s, length: %d bytes (%.2f MB)",
                type(raw_email),
                len(raw_email) if raw_email else 0,
                email_size_mb,
            )
            ...
            if email_size_mb > self.settings.max_email_size_mb:
                logger.warning(
                    "Email too large (%.2f MB > %.2f MB), skipping",
                    email_size_mb,
                    self.settings.max_email_size_mb,
                )
                return None
            ...
            attachments = []
            if (
                self.settings.extract_attachments
                and email_size_mb < self.settings.max_email_size_mb
            ):
                attachments = self.extract_attachments(msg)
            else:
                logger.info(
                    "Skipping attachment extraction (disabled or email too large)"
                )
    ...
    def extract_sender_info(self, msg: EmailMessage) -> Dict[str, str]:
        ...
        logger.info(
            "Extracted sender: %s <%s>", clean_str(sender_name), clean_str(sender_email)
        )
    ...
    def extract_email_body(self, msg: EmailMessage) -> str:
        ...
        if msg.is_multipart():
            ...
            for part in msg.walk():
                ...
                if (
                    parts_processed > 50
                ):  # Safety limit: don't process more than 50 parts
                    logger.warning(
                        "Email has too many parts (>50), stopping body extraction"
                    )
                    break
                ...
                if content_type == "text/plain":
                    ...
                    try:
                        ...
                        if body_bytes:
                            ...
                            if len(body) > max_body_size:
                                logger.warning(
                                    f"Body too large ({len(body)} bytes), truncating to {max_body_size} bytes"
                                )
                                body = body[:max_body_size] + "... [truncated]"
                        logger.info(
                            "Plain text body length: %d", len(body) if body else 0
                        )
                        if body:
                    ...
                    try:
                        ...
                        if body_bytes:
                            ...
                            if len(body) > max_body_size:
                                logger.warning(
                                    f"HTML body too large ({len(body)} bytes), truncating to {max_body_size} bytes"
                                )
                                body = body[:max_body_size] + "... [truncated]"
                        logger.info(
                            "HTML body length after conversion: %d",
                        ...
                        if filename:
                            ...
                            try:
                                ...
                                if isinstance(payload, list):
                                    ...
                                    content_size = sum(
                                        len(str(p)) for p in payload[:10]
                                    )  # Sample first 10 parts
                                else:
                                ...
                                if content_size > max_size_bytes:
                                    logger.info(
                                        f"Skipping large attachment: {filename} (~{content_size / (1024*1024):.2f} MB > {self.settings.max_attachment_size_mb} MB)"
                                    )
                                    attachments.append(
                                        {
                                            "filename": filename,
                                ...
                                if content and len(content) > max_size_bytes:
                                    logger.info(
                                        f"Attachment {filename} decoded but too large ({len(content) / (1024*1024):.2f} MB), truncating metadata"
                                    )
                                    attachments.append(
                                        {
                                            "filename": filename,
                                    ...
                                    logger.info(
                                        f"Found attachment: {filename} ({content_type}, {len(content) / 1024:.1f} KB)"
                                    )

                            except Exception as e:
                                logger.warning(
                                    f"Error processing attachment {filename}: {e}, skipping"
                                )
                                continue
