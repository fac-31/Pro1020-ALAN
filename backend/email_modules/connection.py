...
class EmailConnection:
    ...
    def search_unread_emails(self, mail: imaplib.IMAP4_SSL) -> List[bytes]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Unicode error in search, trying alternative approach: %s",
                    safe_error,
                )
            ...
            if status == "OK" and messages[0]:
                try:
                    ...
                    safe_error = unicodedata.normalize("NFKC", str(e)).replace(
                        "\xa0", " "
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            "Found %d unread emails (after Unicode fix)", len(email_ids)
                        )
                        return email_ids
                    except Exception as e2:
                        safe_error2 = unicodedata.normalize("NFKC", str(e2)).replace(
                            "\xa0", " "
                        )
    ...
    def fetch_email(self, mail: imaplib.IMAP4_SSL, email_id: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            logger.info(
                "Fetch status for email %s: %s",
                clean_str(str(email_id)),
                clean_str(str(status)),
            )

            if status == "OK" and msg_data and msg_data[0]:
                email_body = msg_data[0][1]
                logger.info(
                    "Email body type: %s, length: %d",
                    type(email_body),
