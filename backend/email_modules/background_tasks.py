...
async def email_polling_task(
    email_client: EmailService, rag_service: RAGService = None
):
            ...
            for email in unread_emails:
                try:
                    ...
                    if message_id and message_id in processed_ids:
                        logger.info(
                            f"Email message {message_id} already processed, skipping"
                        )
                        continue
                    ...
                    if (
                        ...
                        try:
                            ...
                            if success_add:
                                logger.info(
                                    f"Added email content to knowledge base: {evaluation.reasoning}"
                                )
                            else:
                                logger.warning(
                                    "Failed to add email content to knowledge base"
                                )

                        except Exception as e:
                            logger.error(
                                f"Error adding email content to knowledge base: {e}"
                            )

                    # Generate reply
                    reply_body = email_client.generate_reply(
                        email["sender_name"],
                        email["sender_email"],
                        email["subject"],
                        email["body"],
                    ...
                    if success:
                        ...
                        logger.info(
                            f"Successfully processed and replied to email from {email['sender_email']}"
                        )
                    else:
                    ...
                    logger.error(
                        f"Error processing email from {email.get('sender_email', 'unknown')}: {e}"
                    )
                    continue
