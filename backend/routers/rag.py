...
def get_digest_service(request: Request) -> DailyDigestService:
    ...
    if (
        not hasattr(request.app.state, "digest_service")
        or not request.app.state.digest_service
    ):
        raise HTTPException(
...
async def upload_document(
    document: DocumentUpload, rag_engine: RAGService = Depends(get_rag_engine)
):
...
async def add_news_article(
    article: NewsArticle, rag_engine: RAGService = Depends(get_rag_engine)
):
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": f"News article '{article.title}' added successfully",
            }
        else:
...
async def search_knowledge_base(
    query_request: QueryRequest, rag_engine: RAGService = Depends(get_rag_engine)
):
    """Search Alan's knowledge base using RAG"""
    try:
        results = rag_engine.search_documents(
            query=query_request.query, n_results=query_request.n_results
        )
...
async def get_faiss_chunks():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error loading FAISS chunks: {str(e)}"
        )


@router.get("/knowledge-base/stats", tags=["RAG"])
async def get_knowledge_base_stats(rag_engine: RAGService = Depends(get_rag_engine)):
...
async def subscribe_to_digest(
    user: UserDigest, digest_service: DailyDigestService = Depends(get_digest_service)
):
    """Subscribe to Alan's daily digest"""
    try:
        success = digest_service.add_user(
            email=user.email, interests=user.interests, name=user.name
        )

        if success:
            return {
                "status": "success",
                "message": f"Subscribed {user.email} to daily digest",
            }
        else:
            raise HTTPException(
                status_code=500, detail="Failed to subscribe to daily digest"
            )
...
async def unsubscribe_from_digest(
    email: str, digest_service: DailyDigestService = Depends(get_digest_service)
):
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": f"Unsubscribed {email} from daily digest",
            }
        else:
            raise HTTPException(
                status_code=404, detail=f"User {email} not found in digest list"
            )
...
async def get_digest_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting digest stats: {str(e)}"
        )
...
Be informative, concise, and helpful."""
        ...
        messages = [
            {
                "role": "system",
                "content": "You are Alan, an AI assistant. Provide helpful, accurate responses based on your knowledge base.",
            },
