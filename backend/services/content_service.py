...
class ContentEvaluationService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(
                "Content Evaluation Service initialized successfully with LangSmith tracking"
            )
        ...
        if settings.langsmith_api_key:
            try:
                ...
                project_name = (
                    ...
                    or f"{settings.langsmith_project}-evaluation"
                )
                ...
                logger.info(
                    f"LangSmith tracking enabled for content evaluation project: {project_name}"
                )
            ...
            logger.info(
                "LangSmith tracking disabled for content evaluation (LANGSMITH_API_KEY not set)"
            )
    ...
    async def evaluate_email_content(
        ...
        try:
            ...
            return ContentEvaluation(
                ...
                extracted_content=evaluation_result.get(
            ...
            raise create_content_evaluation_error(
                f"Content evaluation failed: {str(e)}"
            )
                ...
                if content:
                    if content_type.startswith("text/"):
                        try:
                            ...
                            logger.warning(
                                f"Failed to extract text from attachment {filename}: {e}"
                            )
                            content_parts.append(
                                f"File: {filename} (text extraction failed)"
                            )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        for selector in content_selectors:
                            ...
                            if elements:
                                main_content = " ".join(
                                    [elem.get_text().strip() for elem in elements[:3]]
                                )
                                break
