...
class EmailService:
    ...
    def check_unread_emails(self) -> List[Dict]:
        ...
        try:
            ...
            if not mail:
                raise create_email_connection_error("Failed to establish IMAP connection")
            ...
            for i, email_id in enumerate(email_ids[:max_emails]):
                try:
                    logger.info("Processing email %d/%d: %s", i+1, max_emails, clean_str(str(email_id)))
                    ...
                    if email_body:
                        ...
                        if parsed_email:
                            ...
                            logger.info("Successfully parsed email from %s", 
                    ...
                    logger.warning("Unicode error processing email %s, skipping: %s", 
                                 clean_str(str(email_id)), clean_str(str(e)))
                    continue
                except Exception as e:
                    logger.error("Error processing email %s: %s", 
                               clean_str(str(email_id)), clean_str(str(e)))
                    continue
    ...
    def generate_reply(self, sender_name: str, sender_email: str, subject: str, body: str) -> str:
        """Generate email reply using AI service"""
        try:
            return self.reply_generator.generate_reply(sender_name, sender_email, subject, body)
        except Exception as e:
