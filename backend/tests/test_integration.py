...
class TestEmailProcessingIntegration(unittest.TestCase):
    ...
    def test_content_evaluation_integration(self, mock_chat_openai, mock_openai):
        ...
        evaluation = asyncio.run(
            evaluator.evaluate_email_content(
    ...
    @patch("services.content_service.ChatOpenAI")
    @patch("services.rag_service.OpenAI")
    def test_complete_email_processing_pipeline(
        self, mock_rag_openai, mock_chat_openai, mock_openai
    ):
        """Test complete email processing pipeline"""
        # Mock OpenAI clients
        mock_client = Mock()
        ...
        rg = ReplyGenerator()
        reply = rg._generate_fallback_reply(
            parsed_email["sender_name"], parsed_email["subject"]
        )
...
class TestBackgroundTaskIntegration(unittest.TestCase):
    ...
    def test_email_polling_task_integration(
        self, mock_generate_reply, mock_rag_openai, mock_chat_openai, mock_openai
    ):
